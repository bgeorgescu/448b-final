...
public class MultiSeriesQuery {
	...
	public static class FilteredBucketed {
		//restrict to a set of documents
		QueryExpression filter_;
		//compute these values
		QueryExpression series_[];
		//but sliced into these divisions
		QueryExpression buckets_[];
	}
		...
		public ClauseResults evaluateClauses(Aggregations aggr) throws SQLException {
			...
			if(aggr.filter_ != null) {
				for(int i = 0; i < aggr.buckets_.length; ++i) {
					aggr.buckets_[i].terms_ = MultiSeriesQuery.combineExpressions(aggr.buckets_[i].terms_, aggr.filter_.terms_);
				}
			}
	...
	public static class TEST {
		...
		@Consumes("application/json")
		@Produces("application/json")
		public X tallyDocs(X x) throws SQLException {
			return x;
		}
	}
}
