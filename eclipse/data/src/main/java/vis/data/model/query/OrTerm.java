...
public class OrTerm extends NAryTerm {
	...
	public Pair<int[], int[]> compute() throws SQLException {
		...
		for(int i = 1; i < parameters_.terms_.length; ++i) {
			...
			partial = CountAggregator.or(partial.getKey(), partial.getValue(), operand.getKey(), operand.getValue());
		}
